...
int main() {
  ...
  for (int frame_idx = 0; frame_idx < frames_count; frame_idx++) {
    ...
    // 创建新帧并填充基本信息
    sensor::LidarData *frame = data_bag.add_frames();
    ...
    for (int pt_idx = 0; pt_idx < points_per_frame; pt_idx++) {
      ...
      point.set_offset_time(timestamp + pt_idx * 1000); // 递增时间偏移
      point.set_tag(pt_idx % 10);
      point.set_line(pt_idx % 32);
